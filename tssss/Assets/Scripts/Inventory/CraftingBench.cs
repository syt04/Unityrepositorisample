...
public class CraftingBench : Inventory
{
    ...
    public static CraftingBench Instance
    {
        get
        {
            if(instance == null) { instance = FindObjectOfType<CraftingBench>(); }
            return instance;
        }
    }
    ...
    public override void CreateLayout()
    {
        ...
        if (!GameObject.Find("CraftButton"))
        {
            ...
            btnRect.localPosition = inventoryRect.localPosition + new Vector3(slotPaddingLeft, -slotPaddingLeft * 4 - (slotSize * 3));

            btnRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, ((slotSize * 2) - 9 + (slotPaddingLeft * 1)) * InventoryManager.Instance.canvas.scaleFactor);
