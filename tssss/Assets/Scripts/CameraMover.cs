...
public class CameraMover : MonoBehaviour
{
    ...
    [SerializeField]
    private NewBehaviourScript map;
    //カメラ操作の有効無効
  ...
  //  private Vector3 _presentCamPos;
    ...
    private void CameraPositionKeyControl()
    {
        ...
        if (Input.GetKey(KeyCode.RightArrow)|| Input.GetKey(KeyCode.D)) { campos += _camTransform.right * Time.deltaTime * _positionStep; }
        if (Input.GetKey(KeyCode.LeftArrow)|| Input.GetKey(KeyCode.A)) { campos -= _camTransform.right * Time.deltaTime * _positionStep; }
        if (Input.GetKey(KeyCode.UpArrow)|| Input.GetKey(KeyCode.W)) { campos += _camTransform.up * Time.deltaTime * _positionStep; }
        if (Input.GetKey(KeyCode.DownArrow)|| Input.GetKey(KeyCode.S)) { campos -= _camTransform.up * Time.deltaTime * _positionStep; }
